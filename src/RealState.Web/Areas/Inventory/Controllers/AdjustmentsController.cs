...
public class AdjustmentsController : Controller
{
    ...
    public AdjustmentsController(IApplicationDbContext db, IInventoryEngine engine) { _db = db; _engine = engine; }
    ...
    public async Task<IActionResult> Index(DateTime? from, DateTime? to, CancellationToken ct)
    {
        ...
        var q = _db.InventoryAdjustments.AsQueryable();
    ...
    public async Task<IActionResult> Excel(DateTime? from, DateTime? to, CancellationToken ct)
    {
        ...
        return RealState.Web.Common.Xlsx.File($"تسويات المخزون {DateTime.Now:yyyy-MM-dd}.xlsx", "تسويات المخزون", h, rows);
    }
    ...
    private async Task<(List<string> Headers, List<object?[]> Rows)> BuildExportAsync(DateTime? from, DateTime? to, CancellationToken ct)
    {
        ...
        return (new() { "الرقم", "التاريخ", "المخزن", "الأسطر", "القيمة", "الحالة" }, rows);
    }
    ...
    public async Task<IActionResult> Create(CancellationToken ct)
    {
        ...
        var model = new AdjustmentFormModel { WarehouseId = await _db.Warehouses.Where(w => w.IsDefault).Select(w => w.Id).FirstOrDefaultAsync(ct) };
    ...
    {
        if (!CanDoc()) return Forbid();
        var d = await _db.InventoryAdjustments.Include(x => x.Lines).FirstOrDefaultAsync(x => x.Id == id, ct);
    ...
    public async Task<IActionResult> Save(AdjustmentFormModel model, string? linesJson, CancellationToken ct)
    {
        if (!CanDoc()) return Forbid();
        var lines = Parse(linesJson).Where(l => l.ProductId != Guid.Empty && l.QuantityDelta != 0).ToList();
        if (model.WarehouseId == Guid.Empty) ModelState.AddModelError(string.Empty, "اختر المخزن.");
        if (lines.Count == 0) ModelState.AddModelError(string.Empty, "أضف سطرًا واحدًا على الأقل بكمية غير صفرية.");
        ...
        {
            d = new InventoryAdjustment { Number = await _engine.NextNumberAsync(_db.InventoryAdjustments.Select(x => x.Number), _db.InventoryAdjustments.Local.Select(x => x.Number), model.Date.Year, ct) };
            _db.InventoryAdjustments.Add(d);
        }
        else
        {
            d = await _db.InventoryAdjustments.FirstOrDefaultAsync(x => x.Id == model.Id, ct);
            if (d is null) return NotFound();
            if (d.Status != InventoryDocStatus.Draft) { TempData["ErrorMessage"] = "لا يمكن تعديل مستند مُرحَّل."; return RedirectToAction(nameof(Index)); }
            _db.InventoryAdjustmentLines.RemoveRange(await _db.InventoryAdjustmentLines.Where(l => l.InventoryAdjustmentId == d.Id).ToListAsync(ct));
        }
    ...
    public async Task<IActionResult> Post(Guid id, CancellationToken ct)
    {
        ...
        try { await _engine.PostAdjustmentAsync(d, ct); await _db.SaveChangesAsync(ct); TempData["StatusMessage"] = $"تم ترحيل التسوية {d.Number}."; }
    ...
    {
        if (!CanDoc()) return Forbid();
        var d = await _db.InventoryAdjustments.FirstOrDefaultAsync(x => x.Id == id, ct);
        if (d is null) return NotFound();
        ...
        try
        {
            await _engine.ReverseAsync(InventorySources.InventoryAdjustment, d.Id, ct);
            d.Status = InventoryDocStatus.Reversed;
            await _db.SaveChangesAsync(ct);
            TempData["StatusMessage"] = $"تم عكس التسوية {d.Number}.";
        }
        catch (InvalidOperationException ex) { TempData["ErrorMessage"] = ex.Message; }
        catch (DbUpdateConcurrencyException) { TempData["ErrorMessage"] = "تم تعديل المستند من جلسة أخرى — أعد تحميل الصفحة وحاول مجددًا."; }
        return RedirectToAction(nameof(Index));
    ...
    public async Task<IActionResult> Delete(Guid id, CancellationToken ct)
    {
        ...
        _db.InventoryAdjustmentLines.RemoveRange(d.Lines);
        _db.InventoryAdjustments.Remove(d);
        await _db.SaveChangesAsync(ct);
        TempData["StatusMessage"] = $"تم حذف مسودة التسوية {d.Number}.";
        return RedirectToAction(nameof(Index));
    }
    ...
    // ---- helpers ----
    private List<AdjustmentLineInput> Parse(string? json)
    {
        if (string.IsNullOrWhiteSpace(json)) return new();
        try { return JsonSerializer.Deserialize<List<AdjustmentLineInput>>(json, JsonOpts) ?? new(); } catch { return new(); }
    }
